 ...
 * @Description:
 * @Author: YJL
 * @Date: 2022-07-16 09:30:49
 * @LastEditors: YJL
 * @LastEditTime: 2022-07-16 10:46:35
 */
...
export default class App extends React.Component<
  { children?: React.ReactNode; delay: any },
  { currentStyleCode: any; finished: any; heartRains: any }
> {
  fullStyle = [
    `/*
* Hi。各位！
...
.styleEditor {
  ...
  -webkit-transform:perspective(1000px) rotateY(10deg) translateZ(-100px);`
      : `transform:perspective(500px) rotateX(10deg) translateZ(-100px) translateY(-35px)
  -webkit-transform:perspective(500px) rotateX(10deg) translateZ(-100px) translateY(-35px)`
  } ${
      isPc
        ? ''
        : `
...
.heartWrapper {
  ...
  ${
    isPc
      ? `transform:perspective(1000px) rotateY(-10deg) translateZ(-100px);
  -webkit-transform:perspective(1000px) rotateY(-10deg) translateZ(-100px);`
      : `transform:perspective(500px) rotateX(10deg) translateZ(-100px);
  -webkit-transform:perspective(500px) rotateX(10deg) translateZ(-100px);`
  }${
      isPc
        ? ''
        : `
...
`,
  ...
  state: { currentStyleCode: any; finished: any; heartRains: any } = {
    currentStyleCode: '',
    finished: false,
    heartRains: [],
  };
  ...
  saveStyleEditorRef = (child: any) => (this.styleEditor = child);
  ...
  removeRain(time: number) {
    let { heartRains } = this.state;
    heartRains = heartRains.filter((item: { time: any }) => item.time !== time);
    this.setState({ heartRains });
  }
  ...
  render() {
    ...
    return (
      <>
        ...
        {heartRains.map(
          (item: { rainNum: any; time: React.Key | null | undefined }) => (
